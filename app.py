...
with c1:
    with st.container(border=True):
        st.markdown('<div style="font-size:18px; color:#005a70; font-weight:600; margin:0 0 8px 0;">STEP 1: CHOOSE A QUARTER RANGE</div>', unsafe_allow_html=True)
        ...
        if _quarter_key(q_start) > _quarter_key(q_end): q_start, q_end = q_end, q_start
        st.caption("Filter indicators by a quarter range (e.g., 2025Q1–2025Q4).")
        compare_mode = st.radio("Compare vs:", ["Previous period (equal length)", "Same quarters last year (YoY)"], index=0,
